...
/*!50001 VIEW `job_view` AS select `c`.`Name` AS `Client`,`j`.`description` AS `Description`,`j`.`Status` AS `Status`,`a`.`Name` AS `Architect`,`e`.`Name` AS `Engineer`,`b`.`Name` AS `Builder` from ((((`client` `c` join `job` `j`) join `architect` `a`) join `engineer` `e`) join (`builder` `b` join `job_client` `jc`)) where ((`jc`.`Job_ID` = `j`.`Job_ID`) and (`jc`.`Client_ID` = `c`.`Client_ID`) and (`a`.`Arch_ID` = `j`.`Arch_ID`) and (`e`.`Eng_ID` = `j`.`Eng_ID`) and (`b`.`Builder_ID` = `j`.`Build_ID`) and (`jc`.`Client_ID` = 2)) */;
...
-- Dump completed on 2014-03-18  0:14:24
