...
public class EditCharacter extends JInternalFrame {
	...
	private static String[] engNames;
	...
	public EditCharacter() {
		 ...
		 * Create new array of sufficient size Add relevant information for each
		 * row to the array
		 */
		...
		try {
			...
			charArray = new Object[count][3];
			rs = dbc.retrieveEngineerDetails();
			engNames = new String[count];
			...
			while (rs.next()) {
				...
				engNames[count] = (String) charArray[count][1];
				count++;

			}

			String[] comboArray = engNames;
		...
		jbtUpdate.setBounds(108, 233, 85, 20);
		contentPanel.add(jbtUpdate);
